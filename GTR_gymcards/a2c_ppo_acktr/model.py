...
class VLMPolicy(nn.Module):
    ...
    def act(self, inputs, deterministic=False, INPUT_IDS=None):
        ...
        value, output_ids, text_action, action_log_prob, action_tokens_log_prob, raw_output_ids = llava_generate(value_model = self.value_model,
                                                    tokenizer = self.tokenizer,
                                                    input_ids = INPUT_IDS,
                                                    image_tensor = image_tensor,
                                                    args = self.args)
        action, legal_action = self.projection_f(text_action)
        return value, output_ids, action, action_log_prob, action_tokens_log_prob, legal_action, raw_output_ids

    def sft_forward(self, inputs, input_ids=None, labels=None):
        image_tensor = self.process_obs(inputs)
        if input_ids is None:
            input_ids = self.INPUT_IDS
        if image_tensor.size(0) != 1:
        ...
        image_tensor = image_tensor.to(self.base.device)
        _, _, _, _, inputs_embeds, labels_embeds = self.base.prepare_inputs_labels_for_multimodal(input_ids, None, None, None, labels, image_tensor)
        inputs_embeds = inputs_embeds.to(self.base.device, dtype = self.base.dtype)
        
        outputs = self.base(
            inputs_embeds = inputs_embeds,
            labels = labels_embeds,
            return_dict = True,
        )
