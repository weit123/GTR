...
class RolloutStorage(object):
    def __init__(self, num_env_steps, num_steps, num_processes, obs_shape, action_space, max_new_tokens):
        ...
        self.input_ids = torch.zeros(
            num_env_steps, num_processes, 2*max_new_tokens).long()
    ...
    def to(self, device):
        ...
        # save GPU RAM
        self.obs_accu = self.obs_accu.to('cpu')
    ...
    def feed_forward_generator(self,
        ...
        for sample in sampler:
            ...
            tc_obs_batch = self.obs_accu[:-1].view(-1, *self.obs_accu.size()[2:])[tc_indices].to(self.input_ids.device)
